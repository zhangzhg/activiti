...
public class FileUtils {
    ...
    public static String readFile(String fileName) {
        ...
        try (
             ...
             InputStreamReader insReader = new InputStreamReader(ins, charset);
    ...
    public static String getUploadPath(String root, String fileName, String... subModels) {
        String dateFormat = "yyyy" + File.separator + "MM" + File.separator + "dd";
        String slash = File.separator;
        List<String> models = new ArrayList<>();
        if (null != subModels){
            models.addAll(Arrays.asList(subModels));
        }
        ...
        StringBuilder builder = new StringBuilder(root)
                ...
                .append(slash);
        for (String path : subModels) {
            builder.append(path).append(slash);
        }

        String parentPath = builder.toString();
        parentPath = parentPath.replace("\\\\", File.separator).replace("/", File.separator);
        File file = new File(parentPath);
        if (!file.exists()) {
            file.mkdirs();
        }

        String path = parentPath.concat(fileName);
        return path;
    }
    ...
    public static String getSuffix(String fileName) {
        ...
        if(fileName.contains(point)) {
            suffix = fileName.substring(fileName.lastIndexOf(point) + 1);// 文件的后缀名
        }
