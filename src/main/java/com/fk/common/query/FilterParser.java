package com.fk.common.query;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.alibaba.fastjson.parser.Feature;
import com.fk.common.em.LogicType;
import com.fk.common.em.MatchType;
import com.fk.common.exception.BusinessException;
import com.fk.common.util.ParamUtils;
...
public class FilterParser {
	...
	public FilterParser(String filter) {
		...
		JSONObject filterObject = JSON.parseObject(filter, Feature.OrderedField);
		// 排序处理
	...
	private List<SearchFilter> parse(JSONObject jsonObject, LogicType logicType) {
		...
		for (int i = 0; i < keys.size(); i++) {
			...
			if (key.equalsIgnoreCase("and")) {
				if (!ObjectUtils.isEmpty(searchFilter)) {
	...
	private JSONObject handleJSONArray(JSONArray jsonArray,
		...
		if (ParamUtils.isNotNull(jsonArray)) {
			if (jsonArray.size() < 2) {
				throw new BusinessException(roundLogicType + "查询条件数必须大于2!");
			}
		}
	...
	private SearchFilter createSearchFilter(String field, JSONObject fieldValue) {
		...
		for (String matchType : fieldValue.keySet()) {
			...
			if (value instanceof JSONArray) {
				...
				return new SearchFilter(field, this.toMatchType(matchType), objects);
			} else {
				return new SearchFilter(field, this.toMatchType(matchType), value);
			}
		}
		return null;
	}
